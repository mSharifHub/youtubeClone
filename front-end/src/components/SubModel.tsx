...
export const SubModal: React.FC<SubModelProps> = ({
  isOpen,
  onClickOutside,
    ...
    >
      ...
      <section className="p-2 border-b-[0.5px] ">
        <button
          ...
          <FontAwesomeIcon icon={faArrowLeft} className="text-md p-2 rounded-full  transition-colors duration-75 ease-out hover:bg-neutral-100 dark:hover:bg-neutral-700 cursor-pointer " />
      ...
      <section className="p-2 border-b-[0.5px]">
        <div className=" flex flex-col justify-center items-start h-14 px-2 w-60 text-xs">
          ...
          <span className="text-neutral-400"> {isLoggedIn && user && user.email}</span>
        </div>
      </section>
      {/* Row-3  Accounts List */}
      <section className=" flex justify-start items-center p-2">
        {googleAccounts.length > 0 &&
          googleAccounts.map(({ firstName, lastName, profilePicture, youtubeHandler, subscribersCount }) => (
            <ul
                ...
                <div className="col-start-2 col-span-1 flex flex-col">
                  ...
                  <div className="flex flex-col w-full text-[12px] text-neutral-400">
                    <span className="relative">
                      @{youtubeHandler}
                      {isLoggedIn && user && user.youtubeHandler === youtubeHandler && <span className="absolute right-0"><FontAwesomeIcon icon={faCheck} size="xl"/> </span>}
                    </span>
                    <div>
                      {subscribersCount === 0 && <span>no subscribers</span>}
                      {subscribersCount === 1 && <span>{subscribersCount}<span>subscriber</span> </span>}
                      {subscribersCount > 1 && <span>{subscribersCount} <span>subscribers</span></span>}
                    </div>
                  </div>
                </div>
      ...
      <section className="flex justify-start items-center p-2 border-b-[0.5px] ">
        <div className="h-10 w-full flex justify-start items-center  px-2 text-sm rounded-lg transition-colors duration-75 ease-out hover:bg-neutral-100 dark:hover:bg-neutral-700 cursor-pointer ">View all channels</div>
      </section>
      ...
      <section className="flex  flex-col justify-start items-center p-2">
        <div
          ...
          <FontAwesomeIcon icon={faUserPlus} className="text-sm" /> <span>Add account</span>
        </div>
